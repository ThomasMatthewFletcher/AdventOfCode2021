...
class Scanner:
    def __init__(self, id: int):
        ...
        self.abs_position: Optional[Vector] = None
    ...
    def set_absolute_location(self, rel_pos: Vector, orientation_index: int):
        self.abs_position = rel_pos
        self.abs_beacons = offset_all(self.orientations[orientation_index], rel_pos)
...
def main():
    ...
    while unchecked_scanners and (len(known_scanners) != len(scanners)):
        ...
        assert this_scanner.abs_position
    ...
    for scanner_a in scanners:
        scanner_a_pos = scanner_a.abs_position
        assert scanner_a_pos
        for scanner_b in scanners:
            scanner_b_pos = scanner_b.abs_position
            assert scanner_b_pos
            dist = manhatten_dist(scanner_a_pos, scanner_b_pos)

            if dist > largest_dist:
                largest_dist = dist

    print(f'Answer: {largest_dist}')
...
def manhatten_dist(v1: Vector, v2: Vector) -> int:
