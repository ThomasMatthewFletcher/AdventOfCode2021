...
class Path:
    ...
    def add(self, cave: Cave):
        if (not cave.is_big) and (cave in self.path):
            self.visited_small_twice = True

        self.path.append(cave)

    def can_visit(self, cave: Cave) -> bool:
        return cave.is_big or (cave not in self.path) or (
            (not self.visited_small_twice) and (cave.name != 'start')
        )

    def copy(self) -> Path:
        new_path = Path(self.path)
        new_path.visited_small_twice = self.visited_small_twice
        return new_path
...
class Map:
    ...
    def get_all_paths(self) -> List[Path]:
        ...
        partial_paths: List[Path] = [Path([start_cave])]

        while partial_paths:
            current_path = partial_paths.pop(0)
            current_cave = current_path.path[-1]

            next_caves = current_cave.get_adjacent_caves()
            next_caves = [c for c in next_caves if current_path.can_visit(c)]

            for next_cave in next_caves:
                next_path = current_path.copy()
                next_path.add(next_cave)
